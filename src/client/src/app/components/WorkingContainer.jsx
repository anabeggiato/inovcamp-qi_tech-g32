...
export default function WorkingContainer({ color = "purple", type }) {
    const studentType = [
    ...
    const investorType = [
        { num: 1, title: "Cadastre-se", desc: "Crie sua conta de investidor gratuitamente" },
        { num: 2, title: "Defina seu Perfil", desc: "Escolha valores, cursos e scores de interesse" },
        { num: 3, title: "Explore Oportunidades", desc: "Analise perfis de estudantes disponíveis" },
        { num: 4, title: "Invista", desc: "Escolha em quantos alunos quer investir" },
        { num: 5, title: "Acompanhe", desc: "Monitore desempenho e frequência em tempo real" },
        { num: 6, title: "Receba Retornos", desc: "Pagamentos automáticos direto na sua carteira" },
    ]


    let items

    type === "student" ? items = studentType : items = investorType
            ...
            <div className="grid grid-cols-3 gap-10">
                {items.map((item) => (
                    <div key={item.num} className="flex items-center gap-3">
                        <div className={`flex items-center justify-center text-white text-lg w-12 h-12 rounded-full shrink-0
                                ${color === "purple" ? "bg-purple-gradient" : "bg-second-gradient"}`}>
                            {item.num}
                        </div>
