...
exports.up = async function (knex) {
  ...
  await knex.schema.raw(`
    ...
    BEGIN
      ...
      INSERT INTO ledger (account_type, user_id, amount, dc, ref, meta, created_at, updated_at)
      VALUES (p_account_type, p_from_user, p_amount, 'D', p_ref, p_meta, now(), now());

      INSERT INTO ledger (account_type, user_id, amount, dc, ref, meta, created_at, updated_at)
      VALUES (p_account_type, p_to_user, p_amount, 'C', p_ref, p_meta, now(), now());
    END;
    $$ LANGUAGE plpgsql;
  `);
    ...
    BEGIN
      ...
      LOOP
        ...
        IF v_offer.amount_available <= v_remaining THEN
          INSERT INTO matches (loan_id, offer_id, amount_matched, rate, created_at, updated_at)
          VALUES (p_loan_id, v_offer.id, v_offer.amount_available, v_offer.min_rate, now(), now());
          
          v_remaining := v_remaining - v_offer.amount_available;
          UPDATE offers SET amount_available = 0, updated_at = now() WHERE id = v_offer.id;
        ELSE
          INSERT INTO matches (loan_id, offer_id, amount_matched, rate, created_at, updated_at)
          VALUES (p_loan_id, v_offer.id, v_remaining, v_offer.min_rate, now(), now());
  ...
  await knex.schema.raw(`
    ...
    BEGIN
      ...
      INSERT INTO scores (user_id, score, risk_band, reason_json, created_at, updated_at)
      VALUES (p_user_id, v_score::INT, v_risk, jsonb_build_object('academic_avg', v_academic_avg, 'max_fraud', v_max_fraud), now(), now());
